...
function countCheck(str: String) {
  ...
  console.log(
    `number of vowels ${vowels} and number of consonenets is ${consonants}`
  );
}
...
const strAnagram1 = "car";
const strAnagram2 = "rac";
let flag = true;
const anagramCheck = (str1: string, str2: string) => {
  if (str1.length === str2.length) {
    for (const ch of str1) {
      !str2.includes(ch) ? (flag = false) : {};
    }
  } else flag = false;
...
//Given an object, write a function to check if a property exists.
...
const propCheck = (obj: object, property: string) => {
  property in obj
    ? console.log("property exists")
    : console.log("property does not exist");
};
propCheck(person, "name");
...
// fibonacci();
//removeDupl(str);
//anagramCheck(strAnagram1, strAnagram2);
